...
public class Train extends IndexedPooledBaseData {
	...
	public TrackSignalSegment currentForwardTrackSegment;
	...
	private void updateTrainSpeed(LintfordCore pCore, Track pTrack) {
	...
	public void updateAxleWorldPosition(Track pTrack, TrainAxle pAxle) {
	...
	public void handleDriveAxles(LintfordCore pCore, Track pTrack, TrainAxle pTrainAxle) {
	...
	public void handleUpdateAxles(Track pTrack, TrainAxle pTrainAxle) {
		final var lTrainCar = pTrainAxle.parentTrainCar;
	...
	public void mergeTrains(Track pTrack, Train pOtherTrain, TrainHitch pTrainHitch) {
	...
	private TrainCar detachTrainCar(TrainCar pTrainCar) {
		...
		for (int i = 0; i < lNumberTrainCars; i++) {
			...
			else if (lTrainCar.poolUid == pTrainCar.poolUid) {
	...
	public void reorientateTrainCarsToLocomotive(Track pTrack) {
		// 'Rebuild' the follow map using the last car
		clearFollowPath();
	...
	private void rebuildFollowEdgeList(Track pTrack, TrainAxle pLeadingAxle, boolean pFLipDirection) {
		TrackSegment lLastEdgeAddedToFollowList = null;
		...
		while (lCurrentAxle != null) {
			...
			if (lLastEdgeAddedToFollowList == null || lCurrentAxle.currentEdge.uid != lLastEdgeAddedToFollowList.uid) {
				...
				if (lLastEdgeAddedToFollowList != null) {
					final var lCommonNodeUid = TrackSegment.getCommonNodeUid(lLastEdgeAddedToFollowList, lCurrentAxle.currentEdge);
					if (lCommonNodeUid == -1) {
			...
			if (lLastEdgeAddedToFollowList == null || lOtherAxle.currentEdge.uid != lLastEdgeAddedToFollowList.uid) {
				// Need to get a list of all nodes between the last registered node and the current one
				if (lLastEdgeAddedToFollowList != null) {
					final var lCommonNodeUid = TrackSegment.getCommonNodeUid(lLastEdgeAddedToFollowList, lOtherAxle.currentEdge);
					if (lCommonNodeUid == -1) {
						final var lLastFollowEdge = getLastFollowEdge();
	...
	public void reorientateTrainCarsToLocomotiveOld(Track pTrack) {
		// 'Rebuild' the follow map using the last car
		clearFollowPath();
		TrackSegment lLastEdgeAddedToFollowList = null;
	...
	public TrainFollowEdge addFollowEdge(TrackSegment pEdge, int pDestNodeUid) {
